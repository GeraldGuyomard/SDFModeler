...
INLINE TReturnValue evaluatePrimitive(TEvaluator evaluator, CONSTANT ObjectHeader* header)
{
    ...
    else if (auto ret = computeEvaluationReturn<TEvaluator, SDFTorus, TReturnValue>(evaluator, header))
    {
        return ret.returnValue;
    }
    else if (auto ret = computeEvaluationReturn<TEvaluator, SDFCylinder, TReturnValue>(evaluator, header))
    {
        return ret.returnValue;
    }
